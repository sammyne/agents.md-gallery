...
```
.
├── Cargo.toml          # 项目清单，包含依赖和元数据
├── src/
│   ├── lib.rs          # 库根文件，公共 API 导出
│   ├── error.rs        # 使用 thiserror 的错误类型
│   └── modules/        # 实现模块
├── tests/              # 集成测试
└── examples/           # 示例用法
```
...
本项目使用 `thiserror` crate 实现符合 Rust 习惯的错误处理。这种方法的优点：
